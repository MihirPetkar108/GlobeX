...
DEFAULT_CONFIG_PATH = os.path.join(
    _REPO, "backend", "brain", "models", "destination_ranking", "ranking_config.json"
)


def _load_weights_from_config(config_path: str, fallback: Dict[str, float]) -> Dict[str, float]:
    """
    Loads the `weights` block from ranking_config.json. Falls back to the
    hardcoded `fallback` dict (logging a warning) if the file is missing,
    unreadable, or its `weights` block doesn't parse as a dict of floats.
    Never raises — a config problem must not break the live endpoint.
    """
    try:
        ...
        if not isinstance(weights, dict) or not weights:
            raise ValueError("`weights` block is missing or not a non-empty dict")
        weights = {str(k): float(v) for k, v in weights.items()}
        return weights
    except Exception as exc:
        logger.warning(
            "Could not load ranking weights from %s (%s: %s) — falling back to hardcoded DEFAULT_WEIGHTS",
...
class OpportunityRankingEngine:
    ...
    def __init__(self, weights: Optional[Dict[str, float]] = None, config_path: Optional[str] = None):
        ...
        if weights is not None:
            # Explicit caller-supplied weights always win over the config file.
            self.weights = weights
        else:
            self.weights = _load_weights_from_config(self.config_path, self.DEFAULT_WEIGHTS)
