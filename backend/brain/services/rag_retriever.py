...
def _brain_path(*parts: str) -> str:
    candidate = os.path.join(_REPO_ROOT, "brain", *parts)
    if os.path.exists(candidate):
        return candidate
    return os.path.join(_REPO_ROOT, "backend", "brain", *parts)
...
def _treaty_passages() -> List[Passage]:
    from brain.controllers.compliance_controller import _TREATY_MAP  # noqa: PLC0415
...
def _tariff_csv_passages() -> List[Passage]:
    csv_path = _brain_path("brain_prev", "data_pipeline", "data", "processed", "tariff_features.csv")
...
def _country_sanctions_passages() -> List[Passage]:
    json_path = _brain_path("compliance_data", "current_facts", "country_sanctions_status.json")
...
def _export_controls_passages() -> List[Passage]:
    json_path = _brain_path("compliance_data", "current_facts", "export_controls.json")
    if not os.path.exists(json_path):
        return []
...
def _rules_of_origin_passages() -> List[Passage]:
    json_path = _brain_path("compliance_data", "current_facts", "rules_of_origin.json")
    if not os.path.exists(json_path):
        return []
...
def _sps_tbt_passages() -> List[Passage]:
    json_path = _brain_path("compliance_data", "current_facts", "sps_tbt.json")
    if not os.path.exists(json_path):
        return []
...
def _document_passages() -> List[Passage]:
    from brain.models.compliance.hs6_document_matrix import _PRODUCT_DOCUMENTS  # noqa: PLC0415
    from brain.controllers.compliance_controller import _DEFAULT_DOCUMENTS  # noqa: PLC0415

    passages = []
    for hs6, directions in _PRODUCT_DOCUMENTS.items():
        for direction, docs in directions.items():
            for d in docs:
                req = "mandatory" if d["mandatory"] else "optional"
                text = (
                    f"{direction.capitalize()} Document for HS6 {hs6}: {d['name']} ({req}), "
                    f"issued by {d['issuing_authority']}."
                )
                passages.append(
                    Passage(
                        text=text,
                        source="hs6_document_matrix._PRODUCT_DOCUMENTS",
                        category="document",
                        metadata={"hs6": hs6, "direction": direction, "type": "document", **d},
                    )
                )
...
def _forecasting_passages() -> List[Passage]:
    csv_path = _brain_path("brain_prev", "data_pipeline", "data", "processed", "destination_country_ranking_features.csv")
    if not os.path.exists(csv_path):
        return []
