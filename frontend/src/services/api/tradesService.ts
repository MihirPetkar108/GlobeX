/**
 * FastAPI trade reads plus Express create/list for marketplace trade requests
 * (POST/GET /api/trades), so an importer send is visible on Export Trades.
 */
...
class TradesService {
  ...
  public async createTradeRequest(payload: {
    ...
    agreedPrice: number;
    currency?: string;
    importerId?: string; // Optional: for platform admins creating cross-organization requests
  }): Promise<TradeRecord> {
    ...
    // Only include importer_id if provided (for platform admins)
